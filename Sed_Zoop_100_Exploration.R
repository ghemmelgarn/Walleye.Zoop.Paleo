#This R script explores how it affects our results to only count the first 100 individuals we find on a slide
...
#rename the Sed_Data_Clean so it doesn't mess with my function down below
Sed_Data_Clean_Master <- Sed_Data_Clean

#clean up environment
rm(ID_QAQC, ID_QAQC2, ID_Valid_Check, rows_to_duplicate, Sed_Data, Sed_Data_Clean_Uncorrected_for_pairs, Sed_Data_Clean_Uncorrected_for_pairs_QAQC, Sed_Data_Clean)


#COUNT FUNCTION---------------------------
#This turns the count process into a single function called count_sed_zoops

count_sed_zoops <- function(Sed_Data_Clean) {
...
return(Individual_Count_by_Taxa_Wide)
#end function
}

# #test if function works
test <- count_sed_zoops(Sed_Data_Clean = Sed_Data_Clean_Master)
# #yes it does!
#always need to run that test for the code below


#get a list of the taxa
taxa <- names(test[,2:29])

#APPLY FUNCTION------------------------------

#make a list to store all the lake count dataframes
lake_count_exploration <- list()

for(i in unique(Sed_Data_Clean_Master$LakeName)){
  Sed_Data_Clean_Lake <- Sed_Data_Clean_Master %>% 
    filter(LakeName == i)
  
  #make an empty matrix to store results for that lake
  count_test <- matrix(NA, nrow = nrow(Sed_Data_Clean_Lake), ncol = length(taxa)+2, dimnames = list(NULL, c("LakeName", "RemainsCounted", taxa)))
  
  #make a vector for the number of rows (number of remains) you have
  nremains <- 1:nrow(Sed_Data_Clean_Lake)

  for(j in nremains){
    #iterate through each row in the dataset
    data <- Sed_Data_Clean_Lake[1:j, ]
    
    #save the lakename
    count_test[j, 1] <- i
    #save the number of remains being counted
    count_test[j,2] <- j
    
    #run the counting function and save the output
    taxa_count <- count_sed_zoops(Sed_Data_Clean = data)
    
    #fill in the data for each taxon
    for(k in taxa){
      count_test[j,k] <- ifelse(k %in% names(taxa_count), taxa_count[[k]], 0)
    }
  }
  
  #calculate total number of individuals
  count_total <- as.data.frame(count_test) %>% 
    mutate(across(3:30, as.numeric))
  count_total$TotalIndividuals <- rowSums(count_total[,3:30])
  
  #save this count dataframe in a list
  lake_count_exploration[[i]] <- count_total
}

#rowbind all these separate dataframes together
lake_count_exploration_master <- bind_rows(lake_count_exploration)

#save the output
#write.csv(lake_count_exploration_master, file = paste0("Data/Output/Sediment_Zoop_Taxa_Count_100_explore", Sys.Date(), ".csv"), row.names = FALSE)



#multivariate plot----------------------------
data <- read.csv("Data/Input/Sediment_Zoop_Taxa_Count_100_explore2026-06-19.csv")

#islate the full counts and the first 100 individuals
max_ind <- data %>% 
  group_by(LakeName) %>% 
  summarize(MaxInd <- max(TotalIndividuals),
            .groups = "drop")
data_max_ind <- left_join(data, max_ind, by = "LakeName")
data.filter <- data_max_ind %>% 
  filter(TotalIndividuals == 100 | TotalIndividuals == `MaxInd <- max(TotalIndividuals)`) %>% 
  #there are multiple rows with these individual counts and I only want to keep the first ones:
  distinct(LakeName, TotalIndividuals, .keep_all = TRUE) %>% 
  #remove the lakes that haven't been counted to 100 yet
  filter(TotalIndividuals >= 100) %>% 
  #remove tenmile because we never counted the whole slide
  filter(LakeName != "Tenmile")


rownames(data.filter) <- paste0(data.filter$LakeName, data.filter$TotalIndividuals)

#Calculate relative abundance and remove the non-abundance columns (these are now rownames anyways)
Relative_Abundance <- data.filter[,3:30] / rowSums(data.filter[,3:30])

#Create vector to identify the consistency checks vs. unique slides AND UPDATE AS NECESSARY
Slide.Type <- c(rep(c("100", "All"), 11))
...
#add a vector of lake names
Plot.Data$LakeName <- data.filter$LakeName
...
NMDS <- ggplot(data = Plot.Data, aes(x = NMDS1, y = NMDS2)) +
  geom_point(size = 3, aes(shape = Sample.Type, color = LakeName))+
...
NMDS
#dev.off()
ggsave("sed_zoop_100_test_nmds.png", plot = NMDS, width = 10, height = 8, units = "in", dpi = 300)


#make a dataframe will only the 100s
data.100 <- data.filter %>% 
  filter(TotalIndividuals == 100)

summary(data.100$RemainsCounted)

#CONCLUSION: We are good to only count to 100 individuals!!
#this takes, on average, 240 remains
